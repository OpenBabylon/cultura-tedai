import { asyncForEach, defer, isFunction, isString, noop } from '@cultura-ai/shared';
 ...
 * @param handler - Async or sync function which handles shutdown.
 */
export function onShutdown(dependencies: string[], handler: Handler): CancelCallback;
 ...
 * @param handler - Async or sync function which handles shutdown.
 */
export function onShutdown(name: string, dependencies: string[], handler: Handler): CancelCallback;

export function onShutdown(...args: any[]): CancelCallback {
	const [_name, _dependencies, _handler] = args;

	const handler = isFunction(_name) ? _name : isFunction(_dependencies) ? _dependencies : _handler;
	...
	if (shuttingDown) {
		console.warn('Warn! Attempt to register graceful shutdown while processing terminating:', name);

		return noop;
	}

	if (dependencies.reduce((acc, dep) => acc || testForCycles(dep), false)) {
		throw new Error(`Adding shutdown handler "${name}" will create a dependency loop: aborting`);
	}

	DEPENDENCY_TREE.set(name, new Set([...(DEPENDENCY_TREE.get(name) || []), ...dependencies]));
	...
	return () => {
		const handlersList = (HANDLERS_MAP.get(name) ?? []).filter((v) => v !== handler);

		if (!handlersList.length) {
			HANDLERS_MAP.delete(name);
		}

		const renewDependencies: Set<string> = new Set(
			handlersList
	...
	try {
		...
		if (allHandlers?.length) {
			const timeoutInterval = setInterval(() => {
				console.warn('Warn! graceful shutdown timeout:', name);
			}, 3000);
...
for (const eventName of HANDLED_EVENTS) {
	process.removeAllListeners(eventName).addListener(eventName, onShutdownEvent);
}
