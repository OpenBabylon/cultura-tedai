...
const log = createLogger(import.meta, 'IOC');
...
export default {
	...
	set(id: string, factoryOrClassReference: Factory | AnyFunction, buildInstantly: boolean = false) {
		if (this.isSet(id)) {
		...
		try {
			constructorPath = new URL(captureStackTrace(this.set).split('\n')[0].slice(7)).pathname.split(
				':',
			)[0];
		} catch (_) {}
	...
	async setup({
		...
		generateTypes = false,
		autoImportPatterns = ['./services/**/*.service.{js,mjs,ts,mts}'],
		typeOutput = './ioc.d.ts',
	...
	async writeTypes(outputFilePath: string) {
		...
		for (const [key, { constructorName, constructorPath }] of storage.entries()) {
...
declare module '@cultura-ai/ioc' {
	...
	function ioc<Key extends keyof IocMap>(id: Key): IocMap[Key];
		...
		if (newHash !== oldHash) {
			...
			await fs.writeFile(outputFilePath, `// hash:${newHash}\n${typesContent}\n`);
		}
	},
};
...
function getConstructorName(factoryOrClassReference: Factory | AnyFunction): string {
	return isConstructable(factoryOrClassReference)
