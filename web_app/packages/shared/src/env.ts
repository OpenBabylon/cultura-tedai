...
export function createEnvParser(targetObject: Record<string, string> | Dict<string>) {
	return Object.freeze({
		...
		int(key: string, defValue: number = 0): number {
			...
			// @ts-ignore
			const value = parseInt(targetObject[key]);
		...
		list<T extends 'string' | 'int'>(
			...
			let list: any[] = (targetObject as any)[key].split(',').map((v: any) => v.trim());
